...
void OnPeekFakelag(CUserCmd* pCmd,bool& bSendPacket, int choke_factor)
{
	// credits : @Soufiw , @ikfakof
	static bool m_bIsPeeking = false;
	if (m_bIsPeeking) {
		bSendPacket = !(g_ClientState->m_nChokedCmds < choke_factor);
		if (bSendPacket)
			m_bIsPeeking = false;
		return;
	}
...
void Miscellaneous::Fakelag(CUserCmd *cmd)
{
	...
	int
		tick_times = ((int)(1.0f / g_GlobalVars->interval_per_tick)) / 64,
		choke;


	switch (g_Options.misc_fakelag_type) {
	case 0:
		choke = std::min(g_Options.misc_fakelag_ticks * tick_times, 14);
		break;
	case 1:
		choke = std::clamp(static_cast<int>(std::ceilf(69 / g_LocalPlayer->m_vecVelocity().Length() * g_GlobalVars->interval_per_tick)), 1, 14);
		break;

	case 2:
		//Нужна доработка ибо они нихуя не так работают 
		//Credit's : @Soufiw & @Snake
		OnPeekFakelag(cmd, Snakeware::bSendPacket, g_Options.misc_fakelag_ticks);
		break;

	case 3:
		int standart = g_Options.misc_fakelag_ticks;
		int multi = standart / 2;
		bool _switch = rand() % 2 ? true : false;
		choke = _switch ? std::min(multi * tick_times, 14) : std::min(standart * tick_times, 14);
		break;
	}
	if (g_Options.misc_fakelag && g_Options.misc_fakelag_ticks >= 1)
		should_fakelag = true;

	if (g_Options.misc_fakelag_on_shot && cmd->buttons & IN_ATTACK)
		should_fakelag = true;
	if (!g_Options.misc_fakelag_on_shot && cmd->buttons & IN_ATTACK)
...
void Miscellaneous::LegitResolver(C_BasePlayer * enemy)
{
	...
	if (g_EngineClient->IsInGame() && g_Options.misc_legit_resolver) {
		if (enemy->m_vecVelocity().Length2D() <= 0.7f)
		{
