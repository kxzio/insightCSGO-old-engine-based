...
{
	const auto color = Color(g_Options.color_sound_esp);

	for (auto& i : SoundEsp::Get().sound) {
		if (i.time + (g_Options.sound_esp_time * 1000) <= GetTickCount()) {
			i.time = 0;
			i.origin = { 0.f, 0.f, 0.f };
		}
		else {
			if (i.enemy && !g_Options.esp_enemies_only)
				continue;
			...
			// need animated
			  ...
			  DrawWave(i.origin, g_Options.sound_esp_radius, color);
